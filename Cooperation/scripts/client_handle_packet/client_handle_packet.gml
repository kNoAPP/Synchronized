...
if(in_id == server) {
	...
	while(bytes_buffered >= 5) {
		...
		if(handshake == MSG_HANDSHAKE) {
			...
			// We've lost data somehow! This is a problem with GameMaker reciving mass packets.
			// Try to find the next full packet using the header.
			//
			// Alternatively, if your computer and connection is *super fast* you could also
			// be reaching here if the input_buffer completely fills up. Try increasing the size
			// by factors of 2 until you reach something managable.
